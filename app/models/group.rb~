...
def update_most_viewed
    ...
    self.most_viewed = papers.map{|paper| [paper.id, visits.select{|v| v.paper_id == paper.id}.map{|v| v.count.to_i }.inject(0){|sum, item| sum + item}]}.sort{|x,y| y[1]<=>x[1]}.first(30)
    self.save
    self.most_viewed
end
