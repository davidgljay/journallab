...
before_filter :admin_user,   :only => [:dashboard]
  ...
  def dashboard
    ...
    @nod_discussion_ratio = make_ratio([Comment.where('created_at > ? AND form = ?', Time.now - 1.month, 'comment').votes.count, Question.where('created_at > ?', Time.now - 1.month).votes.count],[Comment.where('created_at > ? AND form = ?', Time.now - 1.month, 'comment').count, Question.where('created_at > ?', Time.now - 1.month).count])
