...
def feed
   ...
   items.flatten!.sort!{|x,y| y.updated_at <=> x.updated_at}.first(20) unless items.empty?
end

# List of papers by summary requests for the lab 
def sumreq_feed
   sumreqs.map{|s| s.get_paper}.uniq.map{|p| [p, p.meta_sumreqs.select{|s| s.group == self}.count]}.sort!{|x,y| y[1] <=> x[1]} unless sumreqs.empty?
end
