...
=end
  ...
  def set_recent_discussions
    ...
    relevantDiscussions = [followPapers + visitedPapers + groupPapers].flatten.select{|p| p.meta_reactions.count > 2}.uniq.flatten.sort{|x,y| y.latest_activity<=>x.latest_activity }
